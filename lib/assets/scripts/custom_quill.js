...
// Detect platform early: on web, DartCallbacks are plain functions;
// on mobile, they are objects with a .postMessage() method.
// Check any known channel to determine which platform we're on.
function detectPlatform() {
    if (typeof SelectionChannel !== 'undefined') {
        return typeof SelectionChannel === 'function';
    }
    if (typeof FocusChanged !== 'undefined') {
        return typeof FocusChanged === 'function';
    }
    return false;
}
...
window.addEventListener('load', () => {
    if (typeof isPlatform === 'undefined') {
        isPlatform = detectPlatform();
    }
    if (typeof ScrollReady !== 'undefined') {
        callChannel(ScrollReady, "Loaded");
    }
});
...
window.addEventListener('scroll', () => {
    ...
    if (typeof GetScrollPosition !== 'undefined') {
        try {
            callChannel(GetScrollPosition, JSON.stringify(positionMap));
        } catch (err) {
...
function formatParser(format) {
    ...
    callChannel(UpdateFormat, JSON.stringify(formatMap));
}
...
function getSelectionRange() {
    ...
    if (range && range.length > 0) {
        ...
        //Send to Flutter
        if (typeof SelectionChannel !== 'undefined') {
            callChannel(SelectionChannel, JSON.stringify({
                index: range.index,
                length: range.length,
        ...
        if (typeof SelectionChannel !== 'undefined') {
            callChannel(SelectionChannel, JSON.stringify({ hidden: true }));
        }
    }
    if (range) {
        var rangeMap = {};
...
function addComment(commentBody, index, length, commentId = null) {
    ...
    sendCommentsToFlutter();
    // Reset so the same range can be re-selected for a new comment
...
function sendCommentsToFlutter() {
    ...
    callChannel(CommentChannel, JSON.stringify(fullComments));
}

//load Comment Store from Flutter side
function loadCommenStore(savedCommentStore) {
    try {
        let commentsList;
        if (typeof savedCommentStore === 'string') {
            commentsList = JSON.parse(savedCommentStore);
        } else {
            commentsList = savedCommentStore;
        }
        if (!Array.isArray(commentsList)) {
            console.error('Expected array, got:', typeof savedCommentStore);
            return {};
        }
        ...
        console.error("Error converting comments list to map:", error);
    }
}
...
async function setHtmlText(htmlString, isWeb, isEnable, savedCommentStore) {
    //  console.log('*****&&&****&&&*****&&&*****&&&&&******&&&******&&&&&*****&&&&****&&&&****');
    try {
        isPlatform = isWeb;
        isEnabled = isEnable;
        ...
        if (isWeb) {
            ...
            const isCustomHtml = htmlString.includes('class="playButton"') || htmlString.includes('Mark as Read');
            //IF THE CUSTOM HTML HAS BEEN SAVED TO THE BACKEND FOR THE MOBILE APPLICATION VERSION
            if (isCustomHtml) {
                window.quilleditor.root.innerHTML = htmlString;
                //Reattach event Listeners for the custom Buttons
                setTimeout(() => {
                    attachCustomButtonListeners();
                }, 100);
            } else {
                //IF THE HTML IS NOT CUSTOM HTML MEANING THAT THE MOBILE EDITING WORK HAS NOT BEEN SAVED
        ...
        if (savedCommentStore !== null && savedCommentStore !== "undefined") {
            loadCommenStore(savedCommentStore);
        }
    } catch (e) {
...
async function replaceVideoWithThumbnail(htmlContent) {
    ...
    for (let video of videos) {
        console.log("CHECKING FOR VIDEOS AND THE LIKES _________________---------------_____________---------");
...
function checkAllVideosLoaded() {
    loadedVideoCount++;
    if (loadedVideoCount >= totalVideoCount) {
        IsLoadingDone(true)
    }
}
setTimeout(() => {
...
// Reattach event listeners to custom buttons after setting innerHTML
function attachCustomButtonListeners() {
    // console.log('Attaching custom button listeners...');
    const editorRoot = window.quilleditor.root;
    // Find all play buttons
    const playButtons = editorRoot.querySelectorAll('.playButton');
    ...
    playButtons.forEach(playButton => {
        ...
        newPlayButton.addEventListener('click', (e) => {
            ...
            // Find the parent div that contains the video URL
            const videoDiv = newPlayButton.closest('div[alt]');
            if (videoDiv) {
                const link = videoDiv.getAttribute('alt');
                //  console.log('Play button clicked, video URL:', link);

                if (!isPlatform) {
                    setTimeout(() => {
                        unFocus();
                    }, 200);
                    callChannel(GetVideoUrl, link);
                } else {
                    // For web platform
    ...
    // Find all "Mark as Read" buttons
    const markButtons = editorRoot.querySelectorAll('button:not(.playButton)');
    console.log('Found', markButtons.length, 'potential mark buttons');
    markButtons.forEach(button => {
        if (button.textContent.includes('Mark as Read') || button.textContent.includes('✅')) {
            // Remove old listener
            ...
            newButton.addEventListener('click', (e) => {
                ...
                // Find the parent div with video URL
                const videoDiv = newButton.closest('div[alt]');
                if (videoDiv) {
                    const link = videoDiv.getAttribute('alt');
                    console.log('Mark as Read clicked, video URL:', link);

                    if (!isPlatform) {
                        callChannel(WatchVideo, link);
                    }

                    // Update button appearance
                    newButton.textContent = '✅ Marked';
...
class ImageBlot extends ImageEmbed {
    static create(value) {
        ...
        if (value.includes('.mp4') || value.includes('youtube')) {
            ...
            // On mobile, we notify Flutter to open a native video player.
            // On web, the browser handles video playback natively.
    ...
    static create(value) {
        ...
        if (value.url.includes('.mp4')) {
            ...
            video.addEventListener('timeupdate', () => {
                ...
                callChannel(GetVideoTracking, JSON.stringify(postMap));
            });
