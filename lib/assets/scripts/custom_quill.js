...
window.addEventListener('scroll', () => {
    ...
    if (isPlatform) {
        //  console.log('Testing the scrolling event in the web version from Javascript');
    ...
    static create(value) {
        ...
        if (value.url.includes('.mp4')) {
            ...
            const onPlayerReady = (event, iframe2) => {
                ...
                console.log('This is the onPlayerReady state in the javascript side');
                console.log(key);
                console.log(JSON.stringify(videoMap))
                if (videoMap.hasOwnProperty(key)) {
                    console.log('This is something that happen when youtube player is ready');
                    console.log(`This is the youtube videoMap ${videoMap}`);
